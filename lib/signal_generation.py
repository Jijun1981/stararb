...
class OriginalKalmanFilter:
    ...
    def initialize(self, x_data: np.ndarray, y_data: np.ndarray) -> None:
        ...
        logger.info(f"初始化完成: beta={self.beta:.6f}, alpha={self.alpha:.6f}, R={self.R:.6f}")
...
def main():
    ...
    if len(signal_period) > 0:
        signal_counts = signal_period['signal'].value_counts()
        print(f"\n信号期信号类型:")
        ...
        if len(z_scores) > 0:
            print(f"\nZ-score统计:")
            print(f"  方差: {np.var(z_scores):.3f}")
            print(f"  |Z|>2比例: {np.mean(np.abs(z_scores) > 2.0)*100:.1f}%")
